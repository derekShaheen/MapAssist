    ...
    class GameMemory
    {
        ...
        public static GameData GetGameData()
        {
            ...
            try
            {
                ...
                if (activeProcessID != 0) // Detected an active window
                {
                    foreach(Process p in processList) // Loop over each D2R process to find which is active
                    {
                        if(p.Id == activeProcessID)
                        {
                            gameProcess = Process.GetProcessById(Convert.ToInt32(activeProcessID)); // Active was found, select that process
                            break; // Exit the loop
                        }
                    }
                }
                ...
                {
                    gameProcess = processList.Length > 0 ? processList[0] : null; // If there was no active window detected, just take the first process from the list
                }
